...
if (intent === "escalation" && !state.awaitingEscalationEmail) {
  ...
  if (state.escalationAttempts === 1) {
    ...
    const kbContext = await getDynamicKBContext(
  cleaned,
  settings.agentId,
...
//   // ✅ NEW: translate spoken number to digits (same pattern as email)
//   try {
//     const translation = await openai.chat.completions.create({
//       model: "gpt-4o-mini",
//       temperature: 0,
...
// Examples:
// - "nine eight seven six..." → "9876543210"
// - "ಒಂಬತ್ತು ಎಂಟು ಏಳು..." → "9876543210"
// - "nau aath saat..." → "9876543210"
// Return ONLY the digits, nothing else. No spaces, no dashes.`
//         },
//         {
//           role: "user",
//           content: cleaned
...
//     await speakText(
//       "Thank you. I have noted your details and our team will contact you soon. Is there anything else I can help you with?",
//       state,
//       exotelWs,
//       isSarvamLang
//         ? { language, voice: settings.sarvamVoice, aggressive: true }
...
//     // await db.query(
//     //   `INSERT INTO meeting_requests (email, phone, call_sid)
//     //    VALUES ($1,$2,$3)`,
//     //   [state.meetingEmail, state.meetingPhone, callSid]
//     // ).catch(err => console.error("Meeting log error:", err.message));
...
// await speakText(
//   "I couldn't detect a valid phone number. Please repeat your phone number.",
//   state,
//   exotelWs,
//   isSarvamLang
//     ? { language, voice: settings.sarvamVoice, aggressive: true }
